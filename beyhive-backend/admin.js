...
document.getElementById('setlistForm').addEventListener('submit', async (e) => {
  ...
  const title = document.getElementById('setlistTitle').value;
  ...
  for (let i = 0; i < songItems.length; i++) {
    ...
    const name = songItem.querySelector('.song-name').value;
    const order = parseInt(songItem.querySelector('.song-order').value);
    const notes = songItem.querySelector('.song-notes').value;
  ...
  try {
    ...
    const status = document.getElementById('setlistFormStatus');
    
    if (result.success) {
      status.textContent = 'Setlist added successfully!';
      status.style.color = 'green';
      document.getElementById('setlistForm').reset();
      // Reset songs container to have one empty song
    ...
    const status = document.getElementById('setlistFormStatus');
    status.textContent = 'Error adding setlist';
    status.style.color = 'red';
    setTimeout(() => { status.textContent = ''; }, 3000);
  }
});

// Fetch and render setlists
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.setlists.length > 0) {
        ...
        result.setlists.forEach(setlist => {
          const tr = document.createElement('tr');
          tr.innerHTML = `
            <td>${setlist.id}</td>
            <td>${setlist.title}</td>
            ...
            <td>
              <button onclick="editSetlist('${setlist.id}')" style="background: #007bff; color: white; border: none; padding: 5px 10px; border-radius: 4px; margin-right: 5px;">Edit</button>
              <button onclick="toggleSetlist('${setlist.id}', ${setlist.isActive})" style="background: ${setlist.isActive ? '#ffc107' : '#28a745'}; color: white; border: none; padding: 5px 10px; border-radius: 4px; margin-right: 5px;">${setlist.isActive ? 'Deactivate' : 'Activate'}</button>
              <button onclick="deleteSetlist('${setlist.id}')" style="background: #dc3545; color: white; border: none; padding: 5px 10px; border-radius: 4px;">Delete</button>
            </td>
          `;
          tbody.appendChild(tr);
        });
...
// Toggle setlist active status
async function toggleSetlist(id, currentStatus) {
  try {
    const response = await fetch(`/api/setlists/${id}/toggle`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      }
...
async function deleteSetlist(id) {
  ...
  try {
    ...
    alert('Error deleting setlist');
  }
}

// Edit setlist (placeholder for now)
function editSetlist(id) {
  alert(`Edit functionality for setlist ${id} will be implemented soon!`);
}
